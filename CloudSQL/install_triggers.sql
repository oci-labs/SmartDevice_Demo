...
BEGIN
  ...
  IF new.cc >= new.ccl AND (select count(*) from valve_alert where valve_sn=new.valve_sn and alert_type='cycle count') = 0 THEN
    INSERT INTO valve_alert (valve_sn, manifold_sn, station_num, detection_time, alert_type) VALUES (new.valve_sn, new.manifold_sn, new.station_num, new.timestamp, 'cycle count');
  END IF;

  IF new.p_fault != 'N' THEN
    INSERT INTO valve_alert (valve_sn, manifold_sn, station_num, detection_time, alert_type) VALUES (new.valve_sn, new.manifold_sn, new.station_num, new.timestamp, 'pressure');
  END IF;

  IF new.leak != 'N' THEN
    INSERT INTO valve_alert (valve_sn, manifold_sn, station_num, detection_time, alert_type) VALUES (new.valve_sn, new.manifold_sn, new.station_num, new.timestamp, 'leak');
  END IF;
